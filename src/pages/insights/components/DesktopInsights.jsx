        ...
        <header className="flex justify-between items-end">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              {"Analisis mendalam seputar arus kasmu." || "Analisis mendalam seputar arus kasmu."}
            </p>
          </div>
          {!isPersonal && (
          ...
          <AnimatedContent distance={40} delay={0.2} direction="vertical">
            <Card className={`${themeBg} border-none shadow-sm`}>
              <CardContent className="p-5 flex gap-4 items-start">
                ...
                <div>
                  <h3 className={`text-base font-bold ${themeColor} mb-2`}>{"Insight Cerdas" || "Insight Cerdas"}</h3>
                  <p className="text-sm opacity-90 leading-relaxed text-foreground">
                    {"Pengeluaran terbesarmu bulan ini ada di kategori" || "Pengeluaran terbesarmu bulan ini ada di kategori"} <span className="font-bold">{getTranslatedCategory(activeData[0].name)}</span>. 
          ...
          <AnimatedContent distance={40} delay={0.3} direction="vertical" className="flex flex-col flex-1 min-h-[350px]">
            <Card className="bg-card border-border shadow-sm overflow-hidden flex flex-col h-full w-full text-foreground">
              <CardHeader className="pb-0 shrink-0">
                ...
                <CardDescription className="text-sm">{"Berdasarkan pengeluaran" || "Berdasarkan pengeluaran"} Rp{(totalExpense/1000000).toFixed(1)} Juta</CardDescription>
              </CardHeader>
              ...
              <CardContent className="p-5 flex flex-col gap-4 relative z-10">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-sm font-bold">{"Master Finansial" || "Master Finansial"}</h3>
                  ...
                  <div className="text-right text-muted-foreground">
                    ...
                    <p className="text-[10px]">{"ke Level 5" || "ke Level 5"}</p>
                  </div>
                </div>
                <div className="space-y-1.5">
                  ...
                  <p className="text-[10px] text-muted-foreground font-medium leading-relaxed">{"Hebat! Pengeluaranmu 15% lebih irit bulan ini (+50 XP). Terus pertahankan!" || "Hebat! Pengeluaranmu 15% lebih irit bulan ini (+50 XP). Terus pertahankan!"}</p>
                </div>
              </CardContent>
            </Card>
          </AnimatedContent>
                ...
                <CardHeader className="pb-2">
                  <CardTitle className="text-base flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-1 text-xs font-bold text-emerald-600 bg-emerald-100 dark:bg-emerald-900/40 px-2.5 py-1 rounded-full">
                      <TrendingDown className="w-3.5 h-3.5" /> 15% {"Lebih Hemat" || "Lebih Hemat"}
                    </div>
                  </CardTitle>
                  <CardDescription className="text-sm">{"Bulan ini vs Bulan lalu" || "Bulan ini vs Bulan lalu"}</CardDescription>
                </CardHeader>
                ...
                <Card className="bg-slate-900 text-white shadow-lg border-none relative overflow-hidden flex flex-col justify-between h-full w-full dark:bg-slate-950">
                  ...
                  <CardContent className="p-6 pt-0 space-y-6 relative z-10 flex-1 flex flex-col justify-between">
                    <div className="grid grid-cols-2 gap-4 border-b border-slate-700/50 pb-5">
                      <div>
                        <p className="text-[10px] text-slate-400 uppercase tracking-wider mb-1.5 font-bold">{"Pemasukan".toUpperCase()}</p>
                        <p className="text-lg font-black text-emerald-400">{formatIDR(summary.total_income)}</p>
                      </div>
                      <div>
                        <p className="text-[10px] text-slate-400 uppercase tracking-wider mb-1.5 font-bold">{"Pengeluaran".toUpperCase()}</p>
                        <p className="text-lg font-black text-rose-400">{formatIDR(summary.total_expense)}</p>
                      </div>
                    </div>
                    <div>
                      <p className="text-[10px] text-slate-400 uppercase tracking-wider mb-1 font-bold">{"Laba Bersih" || "Laba Bersih"}</p>
                ...
                <Card className="bg-card border-border shadow-sm flex flex-col h-full w-full text-foreground">
                  <CardHeader className="pb-3 flex flex-row items-center justify-between">
                    <div>
                      ...
                      <CardDescription className="text-sm">{"Pantau batas pengeluaranmu dari budget" || "Pantau batas pengeluaranmu dari budget"}</CardDescription>
                    </div>
                        ...
                        <div key={idx} className="space-y-2">
                          ...
                          {isOver && (
                            <p className="text-xs font-bold text-rose-500 flex items-center gap-1.5 mt-1">
                              <AlertCircle className="w-3.5 h-3.5" /> {"Peringatan: Anggaran berlebih!" || "Peringatan: Anggaran berlebih!"}
                            </p>
                          )}
                        </div>
                      )
              ...
              <Card className="bg-card border-border shadow-sm flex flex-col w-full h-full text-foreground">
                <CardHeader className="pb-4">
                  <CardTitle className="text-base flex items-center gap-2">
                    ...
                    {"Top 5 Pengeluaran" || "Top 5 Pengeluaran"}
                  </CardTitle>
                  <CardDescription className="text-sm">{"Pengeluaran terbesar bulan ini" || "Pengeluaran terbesar bulan ini"}</CardDescription>
                </CardHeader>
            ...
            <AnimatedContent distance={40} delay={0.8} direction="vertical" className="flex flex-col h-full">
              {isPersonal ? (
                  <Card className="border-dashed border-2 bg-transparent border-border flex flex-col items-center justify-center p-8 text-center h-full min-h-[250px] shadow-none w-full text-foreground">
                    ...
                    <h3 className="text-sm font-bold">{"Jaga Pengeluaran Anda" || "Jaga Pengeluaran Anda"}</h3>
                    <p className="text-xs text-muted-foreground mt-2 max-w-[200px]">{"Tetap berada dalam anggaran bulan ini untuk mencapai target tabungan Anda." || "Tetap berada dalam anggaran bulan ini untuk mencapai target tabungan liburan Anda."}</p>
                  </Card>
              ) : (
                <Card className="bg-card border-border shadow-sm flex flex-col h-full w-full text-foreground">
                  <CardHeader className="pb-4 flex flex-row items-center justify-between">
                    <div>
                      ...
                      <CardDescription className="text-sm">{"Pantau batas pengeluaranmu dari budget" || "Pantau batas pengeluaran budget"}</CardDescription>
                    </div>
                    <Button variant="outline" size="sm" className="h-8 gap-1 text-xs" onClick={() => setIsModalOpen(true)}>
                      <Plus className="w-3.5 h-3.5" /> Budget
                    </Button>
        ...
        <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
          <Card className="w-full max-w-md bg-card border-border shadow-2xl animate-in zoom-in-95 duration-200 text-foreground">
            <CardHeader className="pb-4 border-b border-border flex flex-row items-center justify-between">
              <div>
                <CardTitle className="text-lg">{"Tambah Anggaran Baru" || "Tambah Anggaran Baru"}</CardTitle>
                <CardDescription className="text-sm">{"Atur batas anggaran belanja untuk kategori tambahan." || "Atur batas anggaran belanja untuk kategori tambahan."}</CardDescription>
              </div>
              ...
              <form onSubmit={handleAddBudget} className="space-y-5">
                <div className="space-y-2.5">
                  ...
                  >
                    ...
                    {isPersonal ? (
                      <>
                        <option value="Belanja Bulanan">{"Lain-lain"}</option>
                        <option value="Kesehatan">{"Lain-lain"}</option>
                        <option value="Edukasi & Kursus">{"Lain-lain"}</option>
                        <option value="Investasi">{"Lain-lain"}</option>
                      </>
                    ) : (
                      <>
                        <option value="Gaji Karyawan">{"Gaji Pokok"}</option>
                        <option value="Sewa Tempat">{"Sewa"}</option>
                        <option value="Inventaris Baru">{"Lain-lain"}</option>
                        <option value="Biaya Admin / Pajak">{"Lain-lain"}</option>
                      </>
                    )}
                    <option value="custom" className="font-bold text-blue-600">+ {"Kategori Custom Baru..." || "Kategori Custom Baru..."}</option>
                  </select>
                  {isCustomCategory && (
                ...
                <Button type="submit" className="w-full mt-4 font-bold text-sm bg-blue-600 hover:bg-blue-700 h-10 border-none text-white">{"Simpan Transaksi"}</Button>
              </form>
            </CardContent>
          </Card>
        </div>, document.body
